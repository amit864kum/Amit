...
const envFiles = ['.env', '.env.local', '.dev.vars'].filter((file) => existsSync(resolve(process.cwd(), file)));
...
const required = ['SITE_URL', 'ADMIN_USERNAME', 'ADMIN_PASSWORD_HASH', 'ADMIN_SESSION_SECRET', 'ADMIN_EMAIL', 'RESEND_API_KEY', 'RESEND_FROM_EMAIL', 'NEXT_PUBLIC_TURNSTILE_SITE_KEY', 'TURNSTILE_SECRET_KEY'];
